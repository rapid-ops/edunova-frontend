  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg">+ Add</button>
      </div>

      <div className="max-w-4xl mx-auto p-6">
        <div className="flex gap-2 mb-6">
          {(['students', 'teachers', 'parents'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${tab === t ? 'bg-blue-600 text-white' : 'bg-gray-900 text-gray-400'}`}>
          ...
          <form onSubmit={handleCreate} className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6 space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {['student', 'teacher', 'parent'].map((r) => (
                <button key={r} type="button" onClick={() => setForm({ ...form, role: r })} className={`px-4 py-2 rounded-lg text-sm ${form.role === r ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-400'}`}>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type} value={(form as any)[f.key]} onChange={(e) => setForm({ ...form, [f.key]: e.target.value })} placeholder={f.placeholder} className="w-full bg-gray-800 text-white rounded-lg px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-500" required={f.key !== 'phone'} />
              </div>
            ))}
        ...
        {loading ? <p className="text-gray-400">Loading...</p> : list.length === 0 ? (
